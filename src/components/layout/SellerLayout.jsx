...
import {
  LayoutDashboard, Package, ShoppingBag, Users, Star, Store,
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            >
              ...
              <div>
                <FasoGazWordmark textSize="text-base sm:text-lg lg:text-xl" />
                {user?.businessName && (
            ...
            <div className="lg:hidden flex items-center gap-2">
              <NotificationBell />
              <button onClick={() => setShowProfileMenu(!showProfileMenu)} aria-label="Profil">
          ...
          {showProfileMenu && (
            <>
              <div className="lg:hidden fixed inset-0 bg-black/30 z-40 animate-fade-in" onClick={() => setShowProfileMenu(false)} />
              <div className="lg:hidden absolute top-16 right-3 w-80 max-w-[calc(100vw-24px)] bg-white rounded-2xl shadow-2xl z-50 animate-slide-down border-2 border-secondary-200">
                <div className="p-5">
                    ...
                    <div className="flex items-center gap-4 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-600 flex items-center gap-1">
                          <Store className="h-3 w-3" />{user?.phone}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2">
                    <button onClick={() => { navigate('/seller/profile'); setShowProfileMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-neutral-700 hover:bg-secondary-50 hover:text-secondary-600 rounded-xl transition-colors font-medium">
                      <User className="h-5 w-5" /><span>Modifier le profil</span>
                    </button>
                    <button onClick={() => { navigate('/seller/subscription'); setShowProfileMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-neutral-700 hover:bg-secondary-50 hover:text-secondary-600 rounded-xl transition-colors font-medium">
                      <CreditCard className="h-5 w-5" /><span>Mon abonnement</span>
                    </button>
      ...
      <PushNotificationGate>
        ...
        {showMoreMenu && (
          <>
            <div className="lg:hidden fixed inset-0 bg-black/30 z-40 animate-fade-in" onClick={() => setShowMoreMenu(false)} />
            <div className="lg:hidden fixed bottom-16 left-0 right-0 bg-white rounded-t-3xl shadow-2xl z-50 animate-slide-up border-t-4 border-secondary-500">
              <div className="px-4 py-4">
                <div className="flex items-center justify-between mb-4 pb-3 border-b-2 border-neutral-100">
                  ...
                  <button onClick={() => setShowMoreMenu(false)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                    <X className="h-5 w-5 text-gray-500" />
                  </button>
                </div>
