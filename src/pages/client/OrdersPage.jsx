...
// ✅ MODIFIÉ: Suppression du mode retrait sur place
//            Flux: pending → accepted → in_delivery → completed
//            `expired` groupé avec les annulées
...
import {
  ...
  Loader2, AlertCircle, Star, Truck
} from 'lucide-react';
...
const OrdersPage = () => {
  ...
  // Rechargement initial + au focus de la fenêtre
  useEffect(() => {
    loadOrders();
  ...
  const handleSubmitReview = async (reviewData) => {
    try {
      ...
      await api.reviews.createReview(reviewData);
      setAlert({ type: 'success', message: 'Votre avis a été publié avec succès !' });
      setShowReviewModal(false);
      setOrderToReview(null);
      await loadOrders();
    } catch (error) {
      setAlert({ type: 'error', message: error.message || 'Erreur lors de la publication' });
    } finally {
      setSubmittingReview(false);
    }
  };

  // `expired` groupé avec les annulées/rejetées
  const filteredOrders = selectedStatus === 'all'
    ? orders
    : orders.filter(order => {
        if (selectedStatus === 'cancelled') {
          return ['cancelled', 'rejected', 'expired'].includes(order.status);
        }
  ...
  // ✅ MODIFIÉ: plus de pickup dans les statuts
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      in_delivery: { color: 'bg-indigo-100 text-indigo-800',  icon: Truck,        label: 'En livraison' },
      ...
      expired:     { color: 'bg-orange-100 text-orange-800',  icon: Clock,        label: 'Expirée' },
    };
        ...
        <div className="space-y-3 sm:space-y-4">
          {filteredOrders.map((order) => (
            ...
            >
              ...
              {/* ✅ MODIFIÉ: Toujours afficher livraison (plus de retrait) */}
              <div className="border-t border-gray-100 pt-3 sm:pt-4 mb-3 sm:mb-4">
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4">
                  ...
                  {order.review.sellerResponse && (
                    <div className="border-t border-yellow-300 pt-3">
                      ...
                      <p className="text-xs sm:text-sm text-yellow-900">{order.review.sellerResponse}</p>
                    </div>
                  )}
                </div>
              )}
